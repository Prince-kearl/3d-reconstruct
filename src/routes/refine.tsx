  ...
  head: () => ({
    meta: [
      { title: "DXF2OBJ — Refine Mesh Workspace" },
      {
        name: "description",
        content:
          "Touch up a reconstructed model: manually paint the subject silhouette to fix background removal without regenerating the mesh.",
      },
      { property: "og:title", content: "DXF2OBJ — Refine Mesh Workspace" },
      {
        property: "og:description",
        content: "Mesh refinement workspace with a manual silhouette touch-up brush.",
      },
    ],
  }),
  component: () => (
...
function RefinePage() {
  ...
  return <RefineWorkspace projectId={projectId} />;
}

function RefineWorkspace({ projectId }: { projectId: string }) {
  const {
    logLines,
    progress,
    completedSteps,
